...
def main(email,collection,number,cutoff):
    collection_name = [ 
                   '3landers',
                   'tubby cats',
                   'invisible friends',
    ...
    for i in range(0, 50):
        if collection_name[i] == collection.lower():
            current_collection = collection_name[i]
            current_contract = contract_id[i]
    
    url = "https://opensea.io/assets/0xb4d06d46a8285f4ec79fd294f78a881799d8ced9/2940"
    ...
    nft_title = bs.find("a", class_="styles__StyledLink-sc-l6elh8-0 ekTmzq CollectionLink--link").get_text()
    nft_price = bs.find("div", class_="Overflowreact__OverflowContainer-sc-7qr9y8-0 jPSCbX Price--amount").get_text()
    ...
    subject = f"The price of {nft_title.title()} has fallen"
    body = f"The price of {nft_title.title()} has fallen below {price}. Now might be a good time to buy it. Click on the link to visit the Opensea Page:\n{url}"
    msg = f"Subject: {subject}\n\n\n{body}"
    if price < cutoff:
        server.sendmail("hackathontest30@gmail.com", email, msg)
        

main("mprasad_be20@thapar.edu","3landers",2018,27)